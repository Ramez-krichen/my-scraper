...
def run_example(label: str, url: str, html: str) -> None:
  job = build_job_from_html(url, html)
  print(f"\n=== {label} ===")
  print(job.to_dict())

def main():
  homepage = Path("page_classifier/mock_data/emtop/homepage.html").read_text(encoding="utf-8")
  product_list = Path("page_classifier/mock_data/emtop/product-list.html").read_text(encoding="utf-8")
  product_detail = Path("page_classifier/mock_data/emtop/product-detail.html").read_text(encoding="utf-8")
  ...
  run_example("Product List", "https://emtoptunisie.com/product-category/outils-sans-fil/ponceuse/", product_list)
  run_example("Product Detail", "https://emtoptunisie.com/product/polisseuse-150-42v-1b2ah-emtop-elap42151/", product_detail)
